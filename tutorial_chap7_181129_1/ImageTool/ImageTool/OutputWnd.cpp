...
void COutputList::OnEditCopy()
{
    ...
    CString message;
    GetText(index, message);

    size_t length = (message.GetLength() + 1) * sizeof(TCHAR);
    HGLOBAL data = GlobalAlloc(GMEM_MOVEABLE, length);
    memcpy_s(GlobalLock(data), length, message.LockBuffer(), length);
    GlobalUnlock(data);
