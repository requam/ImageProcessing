...
BEGIN_MESSAGE_MAP(CImageToolDoc, CDocument)
    ...
    ON_COMMAND(ID_FILTER_WEIGHTEDMEAN, &CImageToolDoc::OnFilterWeightedmean)
    ...
    ON_COMMAND(ID_FILTER_UNSHARPMASK, &CImageToolDoc::OnFilterUnsharpmask)
...
void CImageToolDoc::OnFilterMean()
{
    ...
    CONVERT_DIB_TO_IMAGE(m_dib, img);
    IppByteImage res;
    IppFilter::filterMean(img, res);
    CONVERT_IMAGE_TO_DIB(res, dib);

    AfxPrintInfo(_T("[Image Filter Mean] Input image: %s"), GetTitle());
    AfxNewBitmap(dib);
}
...
void CImageToolDoc::OnFilterWeightedmean()
{
    ...
    CONVERT_DIB_TO_IMAGE(m_dib, img);
    IppByteImage res;
    IppFilter::filterWeightedMean(img, res);
    CONVERT_IMAGE_TO_DIB(res, dib);

    AfxPrintInfo(_T("[Image Filter Weighted Mean] Input image: %s"), GetTitle());
    AfxNewBitmap(dib);
}
...
{
    // TODO: Add your command handler code here
    CGaussianDialog dial;
    if (dial.DoModal() == IDOK) {
        CONVERT_DIB_TO_IMAGE(m_dib, img);
        IppFloatImage res;
        IppFilter::filterGaussian(img, res, dial.getSigma());
        CONVERT_IMAGE_TO_DIB(res, dib);

        AfxPrintInfo(_T("[Image Filter Gaussian] Input image: %s, Sigma: %4.2f"), GetTitle(), dial.getSigma());
        AfxNewBitmap(dib);
    }
}
...
void CImageToolDoc::OnFilterLaplacian()
{
    ...
    CONVERT_DIB_TO_IMAGE(m_dib, img);
    IppByteImage res;
    IppFilter::filterLaplacian(img, res);
    CONVERT_IMAGE_TO_DIB(res, dib);

    AfxPrintInfo(_T("[Image Filter Laplacian] Input image: %s"), GetTitle());
    AfxNewBitmap(dib);
}
...
void CImageToolDoc::OnFilterUnsharpmask()
{
    ...
    CONVERT_DIB_TO_IMAGE(m_dib, img);
    IppByteImage res;
    IppFilter::filterUnsharpMask(img, res);
    CONVERT_IMAGE_TO_DIB(res, dib);

    AfxPrintInfo(_T("[Image Filter Unsharp Mask] Input image: %s"), GetTitle());
    AfxNewBitmap(dib);
}
...
void CImageToolDoc::OnFilterHighboost()
{
    ...
    IppByteImage res;
    float alpha = 1.2f;
    IppFilter::filterHighBoost(img, res, alpha);
    CONVERT_IMAGE_TO_DIB(res, dib);

    AfxPrintInfo(_T("[Image Filter High Boost] Input image: %s, alpha: %4.2f"), GetTitle(), alpha);
    AfxNewBitmap(dib);
}
